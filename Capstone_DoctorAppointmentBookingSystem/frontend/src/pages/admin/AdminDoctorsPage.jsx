            ...
            <table className="table table-hover align-middle mb-0">
              ...
              <tbody>
                {filteredDoctors.map((doc) => {
                  ...
                  }[doc.approval_status] || 'bg-secondary-light text-secondary';
                      ...
                      <tr className={isExpanded ? 'table-active-row' : ''} style={{ cursor: 'pointer' }} onClick={() => toggleExpand(doc.user_id)}>
                        ...
                        <td>
                          {doc.is_active ? (
                            ...
                            <span className="text-danger small fw-600 d-flex align-items-center gap-1.5">
                        ...
                        <td className="text-end" onClick={(e) => e.stopPropagation()}>
                          <div className="d-flex justify-content-end gap-2">
                            ...
                            {/* Enable/Disable actions for non-pending */}
                            {doc.approval_status !== 'PENDING' && (
                              <>
                                {doc.is_active ? (
                                  <button
                                    className="btn btn-sm btn-outline-warning d-flex align-items-center gap-1"
                                    onClick={() => handleAction(doc.user_id, deactivateDoctor, 'Doctor account deactivated.')}
                                    disabled={isBusy}
                                  >
                                    <i className="bi bi-dash-circle"></i> Disable Access
                                  </button>
                                ) : (
                                  <button
                                    className="btn btn-sm btn-success d-flex align-items-center gap-1"
                                    onClick={() => handleAction(doc.user_id, activateDoctor, 'Doctor account activated.')}
                                    disabled={isBusy}
                                  >
                                    <i className="bi bi-play-circle"></i> Enable Access
                                  </button>
                                )}
