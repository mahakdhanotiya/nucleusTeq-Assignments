...
async function loadCandidates() {
  try {
    ...
    body.innerHTML = list.map(c =>
      ...
      '<td><button class="btn btn-sm btn-primary" onclick="openStageModal('+c.id+')">Progress</button> ' +
      (c.resumeUrl ? '<a href="'+c.resumeUrl+'" target="_blank" class="btn btn-sm btn-outline">Resume</a> ' : '') +
      '<button class="btn btn-sm btn-danger" onclick="deleteCandidate('+c.id+')">Delete</button></td></tr>'
    ).join("");
...
async function progressStage(e) {
  ...
  try {
    const res = await fetch(API+"/api/interviews/stage-progression", { method:"POST", headers, body:JSON.stringify(body) });
    const data = await res.json();
...
async function deleteCandidate(id) {
  ...
  try {
    const res = await fetch(API+"/candidates/"+id, { method:"DELETE", headers });
    const data = await res.json();
...
async function loadJobs() {
  try {
    ...
    document.getElementById("jobStats").innerHTML = `
      <div class="stat-card"><div class="stat-icon" style="background:#dbeafe"><svg class="stat-svg" viewBox="0 0 24 24"><path fill="#3b82f6" d="M20 6h-4V4c0-1.11-.89-2-2-2h-4c-1.11 0-2 .89-2 2v2H4c-1.11 0-2 .89-2 2v11c0 1.11.89 2 2 2h16c1.11 0 2-.89 2-2V8c0-1.11-.89-2-2-2zm-6 0h-4V4h4v2z"/></svg></div><div><div class="stat-value">${list.length}</div><div class="stat-label">Active Jobs</div></div></div>`;
    const grid = document.getElementById("jobsGrid");
    if (!list.length) { grid.innerHTML='<div class="empty-state">No jobs yet.</div>'; return; }
    grid.innerHTML = list.map(j => `<div class="job-card">
      <div class="job-header"><h4 class="job-title">${j.title}</h4><span class="job-id">J-${j.id}</span></div>
      ...
      <div style="margin-top:12px;display:flex;gap:8px">
        <button class="btn btn-sm btn-outline" onclick="openEditJobModal(${j.id})">Edit</button>
        ${j.isActive === false ? 
          `<button class="btn btn-sm btn-primary" onclick="activateJob(${j.id})">Activate</button>` : 
          `<button class="btn btn-sm btn-outline" onclick="deactivateJob(${j.id})">Archive</button>`
        }
      </div></div>`).join("");
...
async function createJob(e) {
  ...
  const body = {
    title: document.getElementById("jTitle").value, description: document.getElementById("jDesc").value,
...
async function deactivateJob(id) {
  ...
  try {
    const res = await fetch(API+"/jobs/"+id+"/deactivate", { method:"PUT", headers });
    const data = await res.json();
...
window.updateJob = async function(e) {
  ...
  const body = {
    ...
    description: document.getElementById("ejDesc").value,
    skills: document.getElementById("ejSkills").value.split(',').map(s=>s.trim()).filter(s=>s),
    location: document.getElementById("ejLoc").value,
...
window.activateJob = async function(id) {
  ...
  try {
    const res = await fetch(API+"/jobs/"+id+"/activate", { method:"PUT", headers });
    const data = await res.json();
...
async function loadInterviews() {
  try {
    ...
    body.innerHTML = list.map(i => `<tr>
      ...
      <td>${i.focusArea||'--'}</td>
      <td>
        <button class="btn btn-sm btn-secondary" onclick="openAssignPanel(${i.id})">Assign</button>
        <button class="btn btn-sm btn-primary" onclick="openGiveFeedback(${i.id})">Give Feedback</button>
        <button class="btn btn-sm btn-outline" onclick="viewFeedback(${i.id})">View Feedback</button>
        <button class="btn btn-sm btn-danger" onclick="deleteInterview(${i.id})">Delete</button>
      </td></tr>`).join("");
...
window.submitFeedback = async function(e) {
  ...
  try {
    const res = await fetch(API+"/api/feedback", { method: "POST", headers, body: JSON.stringify(body) });
    const data = await res.json();
    if (data.success) {
...
async function openScheduleModal() {
  try {
    const [cRes, jRes] = await Promise.all([fetch(API+"/candidates",{headers}), fetch(API+"/jobs",{headers})]);
    const cData = await cRes.json(); const jData = await jRes.json();
...
async function scheduleInterview(e) {
  ...
  const body = {
    ...
    interviewDateTime: document.getElementById("siDate").value,
    focusArea: document.getElementById("siFocus").value
  };
  try {
...
async function openAssignPanel(interviewId) {
  ...
  try {
    const res = await fetch(API+"/api/panels", { headers });
    const json = await res.json();
    const items = json.data || [];
    document.getElementById("apPanelId").innerHTML = items.length ?
      items.map(p=>'<option value="'+p.id+'">P-'+p.id+' '+p.name+'</option>').join("") :
...
async function assignPanel(e) {
  e.preventDefault();
  const body = {
    interviewId: parseInt(document.getElementById("apInterviewId").value),
    panelId: parseInt(document.getElementById("apPanelId").value),
    focusArea: document.getElementById("apFocus").value
  };
  try {
    const res = await fetch(API+"/api/interviews/assign-panel", { method:"POST", headers, body:JSON.stringify(body) });
    const data = await res.json();
    if (data.success) { toast("Panel assigned!"); closeModal("assignPanelModal"); loadInterviews(); }
    else { document.getElementById("apMsg").className="msg error"; document.getElementById("apMsg").textContent=data.message||"Failed"; }
  } catch(e) { toast("Server error","error"); }
}
...
async function viewFeedback(interviewId) {
  ...
  try {
    const res = await fetch(API+"/api/feedback/interview/"+interviewId, { headers });
    const data = await res.json();
    const list = data.data || [];
...
async function loadPanels() {
  try {
    ...
    body.innerHTML = list.map(p => {
      ...
      const desig = (p.designation||'').replace(/'/g,"\\'");
      ...
      '<td><button class="btn btn-sm btn-secondary" onclick="openEditPanel('+p.id+',\''+org+'\',\''+desig+'\',\''+p.mobileNumber+'\',\''+p.email+'\')">Edit</button> ' +
      '<button class="btn btn-sm btn-danger" onclick="deletePanelMember('+p.id+')">Delete</button></td></tr>';
    }).join("");
  } catch(e) { console.error("loadPanels error:", e); }
}

function openEditPanel(id, org, desig, mobile, email) {
  document.getElementById("epId").value=id; document.getElementById("epOrg").value=org;
  document.getElementById("epDesig").value=desig; document.getElementById("epMobile").value=mobile;
...
async function updatePanel(e) {
  ...
  const body = { email:document.getElementById("epEmail").value, organization:document.getElementById("epOrg").value, designation:document.getElementById("epDesig").value, mobileNumber:document.getElementById("epMobile").value };
  try {
    const res = await fetch(API+"/api/panels/"+id, { method:"PUT", headers, body:JSON.stringify(body) });
    const data = await res.json();
...
async function createPanel(e) {
  e.preventDefault();
  const body = {
    name: document.getElementById("cpName").value, email: document.getElementById("cpEmail").value,
    organization: document.getElementById("cpOrg").value, designation: document.getElementById("cpDesig").value,
    mobileNumber: document.getElementById("cpMobile").value
  };
  try {
    const res = await fetch(API+"/api/panels", { method:"POST", headers, body:JSON.stringify(body) });
    const data = await res.json();
...
async function deletePanelMember(id) {
  ...
  try {
    const res = await fetch(API+"/api/panels/"+id, { method:"DELETE", headers });
    const data = await res.json();
